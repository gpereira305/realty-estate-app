  ...
  return  (
    <Box maxWidth={"980px"} margin={"auto"} p={"4"}>
       ...
       <Flex paddingTop={"2"} alignItems={"center"} justifyContent={"space-between"}>  
        ...
        <Flex  alignItems={"center"} p={"1"} justifyContent={"space-between"} w={"300px"} color={"green.300"}>
            { rooms }<FaBed/>|  { baths }<FaBath/>|  {millify(area)} sqft <BsGridFill/>    
        </Flex>

          <Divider marginTop={"4"}/> 

        <Text fontSize={"lg"} title={title} marginTop={"2"} marginBottom={"4"} fontWeight={"bold"}>
            { title.toUpperCase() }
        </Text>   
        <Divider marginTop={"8"}/>  
       <Flex flexWrap={"wrap"} textTransform={"uppercase"} justifyContent={"space-between"}>
           <Flex justifyContent={"space-between"} w={"400px"} borderBottom={"1px"} borderColor={"gray.200"} p={"3"}>
              <Text>Tipo:</Text>
              <Text fontWeight={"bold"}>{ type === "property" ? 'Imóvel' : type}</Text>
           </Flex>
           <Flex justifyContent={"space-between"} w={"400px"} borderBottom={"1px"} borderColor={"gray.200"} p={"3"}>
              <Text>Propósito:</Text>
              <Text fontWeight={"bold"}>{ purpose === "for-sale" ? 'Venda' : 'Aluguel'}</Text>
           </Flex> 
           {furnishingStatus && (
            <Flex justifyContent={"space-between"} w={"400px"} borderBottom={"1px"} borderColor={"gray.200"} p={"3"}>
              <Text>Mobiliado:</Text>
              <Text fontWeight={"bold"}>{ furnishingStatus  === "furnished" ? "Com mobília" : "Sem mobília"}</Text>
            </Flex>
           )}
       </Flex>
       ...
       <Accordion allowMultiple>
           <AccordionItem>
              <h2>
                <AccordionButton  _expanded={{ bg: '#68D391', color: '' }} paddingLeft={"0"}>
        ...
        <Box>
          {amenities.length > 1 && (
            <Text fontSize={"medium"} fontWeight={"black"} marginTop={"12"} marginBottom={"2"}>
